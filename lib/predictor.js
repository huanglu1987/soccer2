...
function computeRulePrediction(rows) {
  ...
  } else if (ranked[0].key === "draw" || finalProb.draw >= THRESHOLDS.drawMinimum) {
    decision = { type: "draw-single", primaryKey: "draw", secondaryKey: null };
  } else {
    decision = {
      type: "double",
